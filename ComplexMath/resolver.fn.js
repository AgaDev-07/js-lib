import Parser, { multiple_power_var, } from './Parser.class.js';
import { divide, isInt, log } from './functions.js';
...
function resolveEquation(_left, _right, scope) {
    ...
    if (left.type === 'operator') {
        switch (left.value) {
            case '^':
                ...
                if (left.left.type === 'variable' || left.left.type === 'constant') {
                    if (left.right.type === 'number') {
                        ...
                        return resolveEquation(left.left, multiple_power_var(right, divide(1, power)), scope);
                    }
                    return resolveEquation(left.left, {
                        type: 'operator',
                        value: '^',
                        left: right,
                        right: {
                ...
                if (left.right.type === 'variable' || left.right.type === 'constant') {
                    ...
                    if (base.type === 'number') {
                        return resolveEquation(variable, {
                            type: 'operator',
                            value: '/',
            ...
            case '*':
                ...
                if (left.right.type === 'variable' || left.right.type === 'constant') {
                    return resolveEquation(left.right, {
                        type: 'operator',
                        value: '/',
                        left: right,
                        right: left.left,
...
/** @type {import('../@types/ComplexMath/resolver.fn.d.ts').default} */
export default function resolve(source, scope = {}) {
    ...
    if (simplified.length === 1)
        return Parser.evaluate(simplified[0], scope);
    if (simplified.length === 2) {
        const [left, right] = simplified;
        if (hasVariable(left))
            return resolveEquation(left, right, scope);
        if (hasVariable(right))
            return resolveEquation(right, left, scope);
    }
    throw new Error('Invalid equation');
}
/** @typedef {`${string}=${string}`} equation */
